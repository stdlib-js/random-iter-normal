...
function iterator( mu, sigma, options ) {
	...
	if ( !isNumber( mu ) || isnan( mu ) ) {
		throw new TypeError( format( '0qD6v', mu ) );
	}
	if ( !isPositive( sigma ) ) {
		throw new TypeError( format( '0qD72', sigma ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0qD2V', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0qD2t', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
